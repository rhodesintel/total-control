...
class TotalControlVpnService : VpnService() {
    ...
    private fun shouldBlock(domain: String): Boolean {
        // VPN is FAILSAFE layer - only blocks when AccessibilityService is disabled
        // If AS is enabled, it handles fine-grained context-aware blocking
        if (isAccessibilityServiceEnabled()) {
            Log.d(TAG, "AccessibilityService enabled - VPN allows all, AS handles control")
            return false
        }

        // AS disabled - VPN is only defense, block matching domains
    ...
    private fun getBlockedSites(): Set<String> {
        ...
        // DEFAULT BLOCKLIST - Used when AccessibilityService is DISABLED
        // This is the FAILSAFE layer - blocks entire domains
        return setOf(
            // === SOCIAL MEDIA (PRIMARY FOCUS) ===
            // Note: DNS cannot distinguish timeline from DMs (same domain)
            // When AS is disabled, these are fully blocked
            "twitter.com",
            "x.com",
            "instagram.com",
            "facebook.com",
            ...
            // === STREAMING CDNs ===
            // Block CDNs to allow search/browse but block video playback
            "googlevideo.com",      // YouTube video CDN
            "ytimg.com",            // YouTube thumbnails
            "nflxvideo.net",        // Netflix video CDN

            // === FULL STREAMING BLOCKS ===
            // These have no useful browse features, just consumption
            "twitch.tv",
            "tiktok.com",
            "primevideo.com",
            "disneyplus.com",
